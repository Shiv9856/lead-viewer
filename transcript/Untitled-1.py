async def prepare_call_context(metadata: dict, system_prompt: str) -> str:
    """
    Spinny AI Voice Agent — Pre-Call Function (v7.3)
    ...
    v7.3 changes:
      • Added TD_AUTOCANCELLED scenario for yesterday auto-cancelled visits.
      • Added _get_yesterday_cancelled_visit() helper.
      • Added _get_raw_comment_milestone() helper.
      • _get_comment_milestone() now maps td_followup_done_* variants to base
        equivalents so existing S4-S17 scenarios handle them transparently.
    ...
    Routes using:
      ...
      3. comment.milestone from Robo Call Summary
      4. comment.disposition from Robo Call Summary
      ...
      8. Yesterday auto-cancelled TD detection (NEW in v7.3)
    """
    import json
    import logging
    ...
    ALLOWED_PREF_FIELDS = [
        "body_type", "make", "min_price", "max_price",
        "transmission", "fuel_type", "no_of_owners",
    ]
    ...
    def _get_raw_comment_milestone() -> str:
        """Returns the raw, unmapped milestone value from latest Robo Call Summary.
        Used to detect td_followup_done_* variants BEFORE normalization so that
        TD_AUTOCANCELLED can decide whether to re-pitch reschedule or skip."""
        _, robo_data = _find_latest_robo_summary()
        if robo_data:
            return str(robo_data.get("milestone", "")).lower().strip()
        return ""

    def _get_comment_milestone() -> str:
        """Get milestone from the latest Robo Call Summary, with normalization.

        td_followup_done_* variants are emitted by previous calls that already
        ran the TD_AUTOCANCELLED reschedule pitch. They are mapped to their
        base equivalents so the existing SQL-followup scenarios (S4-S17) handle
        them identically — preventing the reschedule script from restarting
        on every subsequent call.
        """
        m = _get_raw_comment_milestone()
        if not m:
            return ""
        # Base variants
        ...
        # td_followup_done_* → map to base bucket
        if m == "td_followup_done_minimal":
            return "minimal_engagement"
        if m == "td_followup_done_pref_coll":
            return "preference_collected"
        if m == "td_followup_done_car_pitched":
            return "car_pitched"
        # td_followup_done_td_scheduled → TD is now scheduled; the early TDC
        # guard in _route_scenario picks it up via the scheduled_visits scan.
        # Returning "" lets routing fall through naturally.
        return ""
    ...
    def _get_yesterday_cancelled_visit() -> dict | None:
        """
        Scan all_visits.0..2 and their test_drives.0..2 for an auto-cancelled
        test drive that was scheduled for YESTERDAY (IST date comparison).
        Returns the most recent matching visit's details, or None.

        Used to trigger the TD_AUTOCANCELLED scenario when:
          - task == 'call_to_schedule_the_visit'
          - no future scheduled visit exists
          - cm is not already td_followup_done_*
        """
        CANCELLED_STATUS = "testdrive-lifecycle-testdrive-lifecycle-testdrive-cancelled"
        now_ist = _now_ist()
        ...
        for v in range(3):
            for td in range(3):
                td_status = _g(f"all_visits.{v}.test_drives.{td}.status_name", "")
                if not td_status:
                    continue
                if CANCELLED_STATUS not in str(td_status).lower():
                    continue
                raw_time = _g(f"all_visits.{v}.scheduled_time", "")
                dt = _parse_to_ist(raw_time)
                if not dt or dt.date() != yesterday_date:
                    continue
                at_home = str(_g(f"all_visits.{v}.at_home", "false")).lower() == "true"
                candidates.append({
                    "visit_index":   v,
                    "td_index":      td,
                    "visit_id":      _g(f"all_visits.{v}.id", ""),
                    "sell_lead_id":  str(_g(f"all_visits.{v}.test_drives.{td}.sell_lead_id", "")),
                    "at_home":       at_home,
                    "hub_name":      _g(f"all_visits.{v}.hub_name", ""),
                    "scheduled_dt":  dt,
                    "time_readable": dt.strftime("%I:%M %p").lstrip("0"),
        ...
        if not candidates:
            return None
        candidates.sort(key=lambda x: x["scheduled_dt"], reverse=True)
        return candidates[0]
        ...
        if (len(scheduled_visits) > 0
            ...
            if len(sv) >= 2:
                ...
                greeting = (
                    ...
                    f'{v1["time_full"]} को {_td_label(v1)} '
                    f'और {v2["time_full"]} को {_td_label(v2)}। '
            ...
            # ── SINGLE TD ─────────────────────────────────────────────────────
                ...
                if v["at_home"]:
                    ...
                    greeting = (
                        ...
                        f'आपने हमारे साथ {v["time_full"]} के लिए '
...
Handle: reschedule / car change / cancel / policy questions."""
                    ...
                    if is_today:
                        greeting = (
                            ...
                            f'आपका {time_full} को {hub} पे Test Drive शेड्यूल्ड है। '
...
Handle: RESCHEDULE / CAR CHANGE / CANCEL."""
        ...
        # ──────────────────────────────────────────────────────────────────────
        # TD_AUTOCANCELLED: Customer's TD got auto-cancelled yesterday because
        # they didn't show up. milestone.task.name == "call_to_schedule_the_visit"
        # but this is actually a reschedule pitch — NOT a fresh SQL followup.
        #
        # Skip if previous call already ran this script
        # (raw cm starts with "td_followup_done_") — fall through to existing
        # SQL-followup scenarios in that case (mapping handled by
        # _get_comment_milestone()).
        # ──────────────────────────────────────────────────────────────────────
        _raw_cm = _get_raw_comment_milestone()
        _is_td_followup_done = _raw_cm.startswith("td_followup_done_")

        if (task == "call_to_schedule_the_visit"
                and not _is_td_followup_done):
            _cancelled_visit = _get_yesterday_cancelled_visit()
            if _cancelled_visit:
                # Fetch car details for the cancelled TD
                _ac_car_info = None
                if _cancelled_visit["sell_lead_id"]:
                    try:
                        _ac_fetched = await _fetch_car_details_from_inventory(
                            [_cancelled_visit["sell_lead_id"]]
                        )
                        if _ac_fetched:
                            _ac_car_info = _ac_fetched[0]
                    except Exception as _ac_err:
                        logger.warning(f"TD_AUTOCANCELLED car fetch failed: {_ac_err}")

                _ac_hub  = _cancelled_visit["hub_name"] or ""
                _ac_time = _cancelled_visit["time_readable"]

                # Greeting variants for at-home vs hub TD
                if _cancelled_visit["at_home"]:
                    _ac_loc_text = "आपकी Home Test Drive"
                elif _ac_hub:
                    _ac_loc_text = f"आपकी test drive Spinny {_ac_hub} hub पे"
                else:
                    _ac_loc_text = "आपकी test drive Spinny hub पे"

                greeting = (
                    f'Hello. <break time="0.5s" /> मैं प्रिया बोल रही हूँ Spinny से। '
                    f'{_ac_loc_text} कल {_ac_time} पे scheduled थी, '
                    f'<break time="0.4s" /> पर ना आने की वजह से वो auto cancel हो गई है। '
                    f'<break time="0.4s" /> क्या मैं इसे reschedule कर दूँ?'
                )

                # Car context for the script body
                if _ac_car_info and _ac_car_info.get("available", True):
                    _ac_car_line = (
                        f"{_to_words(_ac_car_info.get('year',''))} "
                        f"{_ac_car_info.get('make','')} {_ac_car_info.get('model','')} "
                        f"({_format_price(_ac_car_info.get('price',0))}, "
                        f"{_ac_car_info.get('color','NA')}, "
                        f"{_ac_car_info.get('fuel_type','NA')} {_ac_car_info.get('transmission','NA')})"
                    )
                    _ac_avail = "AVAILABLE"
                elif _ac_car_info:
                    _ac_car_line = (
                    ...
                    _ac_avail = "BOOKED / UNAVAILABLE"
                else:
                    _ac_car_line = "(car details not available)"
                    _ac_avail = "UNKNOWN"
...
SCENARIO: TD_AUTOCANCELLED
The customer's test drive was auto-cancelled yesterday because they did not show up.
This is a RESCHEDULE pitch — NOT a fresh SQL followup.

Cancelled TD details:
  Date:         {_cancelled_visit['date_readable']} (yesterday)
  Time:         {_ac_time}
  Type:         {'Home Test Drive' if _cancelled_visit['at_home'] else 'Hub Test Drive'}
  Hub:          {_ac_hub or 'NA'}
  sell_lead_id: {_cancelled_visit['sell_lead_id']}
  Car:          {_ac_car_line}
  Car status:   {_ac_avail}

─────────────────────────────────────
BRANCH 1 — User agrees to reschedule:
─────────────────────────────────────
Bot: "Great <break time=\\"0.3s\\" /> मैं आपको नया slot suggest कर देती हूँ।"

If car is AVAILABLE:
  → proceed to test drive scheduling block for the SAME sell_lead_id ({_cancelled_visit['sell_lead_id']}).

If car is BOOKED / UNAVAILABLE:
  Bot: "वो कार अभी available नहीं है, किसी और ने book कर ली है।
        <break time=\\"0.3s\\" /> मैं आपको similar cars दिखा देती हूँ।"
  → confirm preferences → SECTION 6 (Car Pitching).

─────────────────────────────────────
BRANCH 2 — User wants a different car instead:
─────────────────────────────────────
Bot: "ठीक है, कोई और कार देखना चाहेंगे? <break time=\\"0.3s\\" /> मैं आपके लिए options निकाल देती हूँ।"
→ confirm preferences → SECTION 6 (Car Pitching).

─────────────────────────────────────
BRANCH 3 — User not interested in rescheduling right now:
─────────────────────────────────────
Bot: "कोई बात नहीं <break time=\\"0.3s\\" /> क्या आप कुछ और cars देखना चाहेंगे, या मैं बाद में call कर लूं?"
→ Capture callback time OR move to preference confirmation.

─────────────────────────────────────
BRANCH 4 — User is busy:
─────────────────────────────────────
Bot: "आप अपना convenient time बता दीजिए, मैं उस time connect कर लूंगी।"
→ Capture callback time.

═══════════════════════════════════════════════════════════════
IMPORTANT — at end of this call, set comment.milestone to ONE of:
  • td_followup_done_td_scheduled  → if TD was rescheduled successfully
  • td_followup_done_car_pitched   → if you pitched alternate cars
  • td_followup_done_pref_coll     → if you only collected/confirmed preferences
  • td_followup_done_minimal       → if no meaningful engagement (busy/refused/callback)
This prevents the next call from re-running this auto-cancel reschedule script.
═══════════════════════════════════════════════════════════════"""

                return greeting, script, "TD_AUTOCANCELLED"
        ...
        if (status == "uar_broken_transaction"
            ...
            _bt_entries = _get_interested_cars("dealrequest")
            _bt_car_info = None
            if _bt_entries:
                _bt_lead_id = _bt_entries[0]["lead_id"]  # index 0 = latest
                try:
...
Bot: "मैं देख पा रही हूँ कि आप {_bt_yr} की {_bt_mk} {_bt_md} बुक करने का ट्राय कर रहे थे{_bt_pr_part}। क्या मैं इसमें आपकी कुछ मदद कर दूँ?"
Wait for customer response.

If user agrees or confirms the car:
Bot: "आप बताइए <break time="0.3s" /> आप इसके रिलेटेड डिटेल्स जानना चाहेंगे या फिर इसकी टेस्ट ड्राइव बुक करना चाहेंगे?"
Wait for users reply then based on users reply move to car details block (Section 7) / test drive scheduling block

If user wants details:
Share full car details from (Section 7) then push for test drive booking.

If user wants test drive:
Proceed to test drive scheduling block directly."""
...
Bot: "मैं देख पा रही हूँ कि आप {city} में कार बुकिंग का try कर रहे थे। उसमें कुछ help कर दूँ?"

If the user agrees then search the car user tried to book test drive by strictly proceeding to SECTION 5 — CAR SEARCH 
...
No budget or model available — collect preferences first then move to car pitching flow""")
        ...
        if (cm == "car_pitched"
            ...
            greeting = f'Hello. <break time="0.5s" /> मैं प्रिया बोल रही हूँ Spinny {city} team से। पिछली बार कुछ कार्स के बारे में बात हुई थी — क्या उनमें से किसी के बारे में जानना चाहेंगे, या मैं कुछ और options दिखा दूँ?'
...
#                             gives colour, price, transmission for previously sent WhatsApp cars
    ...
    # ── Step 6: Log ──
    logger.info(
        f"prepare_call_context v7.3: scenario={scenario_id}, city={city}, "
