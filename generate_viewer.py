...
def _extract_cars(row):
    cars = []
    for i in (1, 2, 3):
        mk = _s(row.get(f"car_{i}.make"))
        mo = _s(row.get(f"car_{i}.model"))
        if mk and mo:
            cars.append({
                "make": mk, "model": mo,
                "fuel": _s(row.get(f"car_{i}.fuel")),
                "price": _c(row.get(f"car_{i}.price")),
                "hub": _s(row.get(f"car_{i}.hub_name")),
                "how_to_reach": _s(row.get(f"car_{i}.how_to_reach")),
                "lead_id": _s(row.get(f"car_{i}.lead_id")),
            })
    return cars
...
def _extract_prefs(row, prefix):
    ...
    # Attributes (features)
    attrs = []
    for attr in ("sunroof", "rear-camera", "rear-parking-sensor", "airbags",
                 "alloy-wheels", "power-steering", "power-windows",
                 "keyless-start", "anti-lock-braking-system-abs-1"):
        if _b(row.get(f"preferences.{prefix}.attribute.{attr}")):
            attrs.append(attr.replace("-", " ").replace("1", "").strip().title())
    if attrs: p["features"] = ", ".join(attrs)
    # City (agent only)
    ...
    for i in range(3):
        ...
        if vid or stime:
            visits.append({
                ...
                "how_to_reach": _s(row.get(f"all_visits.{i}.how_to_reach")),
...
def _extract_functions(row):
    fns = []
    for i in range(1, 50):
        name = _s(row.get(f"function[{i}].name"))
        if name:
            fns.append({
                "name": name,
                "success": _b(row.get(f"function[{i}].success")),
                "ts": _s(row.get(f"function[{i}].timestamp")),
            })
...
def build_leads(df):
    ...
    for _, row in df.iterrows():
        ...
        assigned_cars = _extract_cars(row)
        ...
        if assigned_cars: meta["assigned_cars"] = assigned_cars
        ...
        # Car being discussed (for TDC/followup scenarios)
        mm = _s(row.get("make_model"))
        if mm:
            meta["car_context"] = {
                "make_model": mm,
                "price": _c(row.get("car_price")),
                "km": _c(row.get("km_driven")),
                "year": _c(row.get("registration_year")),
                "owner": _c(row.get("owner_count")),
                "rto": _s(row.get("RTO")) or _s(row.get("rto")),
            }
        ...
        # Exchange data
        ex_make = _s(row.get("exchange_data.0.make"))
        if ex_make:
            meta["exchange"] = {
                "make": ex_make,
                "model": _s(row.get("exchange_data.0.model")),
                "variant": _s(row.get("exchange_data.0.variant")),
                "year": _c(row.get("exchange_data.0.make_year")),
            }

        # Functions called
        fns = _extract_functions(row)
        ...
        call = {
            ...
            "scenario": scenario,
            "capability": cap,
            "try_count": _c(row.get("Try Count")),
            # Post-call booleans
            ...
            "callback_req": _b(row.get("callback_requested")),
            "wants_td_2d": _b(row.get("wants_td_after_2_days")),
            "notify_supply": _b(row.get("notify_supply_agent")),
            ...
            "sentiment": _s(row.get("user_sentiment")),
            "dispose": _s(row.get("dispose")),
            "dispose_reason": _s(row.get("disposition_reason")),
            "visit_status": _s(row.get("visit_status")),
            "cancel_reason": _s(row.get("cancellation_reason")),
            "rescheduled_time": _s(row.get("rescheduled_time")),
            "preferred_hub": _s(row.get("preferred_hub")),
            "selected_td_slot": _s(row.get("selected_test_drive_slot")),
            "updated_city": _s(row.get("updated_city")),
            "review": _s(row.get("review")),
            # Car IDs
            "pitched_cars": _s(row.get("pitched_car_ids")),
            "rejected_cars": _s(row.get("rejected_car_ids")),
            "liked_cars": _s(row.get("liked_cars_id")),
            "interested_cars": _s(row.get("user_interested_car_ids")),
        ...
        # Lead-level info (updated each row, last row wins)
        if lid not in leads:
            leads[lid] = {
                "id": lid,
...
<head>
<meta charset="UTF-8">
<title>Lead Journey Viewer v2</title>
...
.tab{padding:8px 16px;font-size:12px;color:var(--tm);cursor:pointer;border-bottom:2px solid transparent;transition:all .1s;font-family:inherit}
...
/* Pref block */
.pblk{background:var(--s);border:1px solid var(--bd);border-radius:4px;padding:14px 16px;margin-bottom:12px}
.plbl{font-family:'Instrument Serif',serif;font-style:italic;font-size:13px;color:var(--tm);margin-bottom:10px;display:flex;align-items:center;gap:6px}
.plbl::before{content:'';width:4px;height:4px;background:var(--ac);border-radius:50%}
...
/* Changed field */
.fv.changed{color:var(--ac);font-weight:500}
/* Cards for assigned cars */
.acards{display:flex;flex-wrap:wrap;gap:6px;margin-top:6px}
.acard{padding:5px 9px;background:var(--s2);border-radius:3px;font-size:11px;border-left:2px solid var(--in)}
.acard b{font-weight:500}.acard .sm{color:var(--tm);margin-left:3px}
/* Timeline */
...
.cscn{font-size:10px;color:var(--tm);font-style:italic;max-width:160px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
...
.fnbox{background:var(--s2);border-radius:3px;padding:8px 10px;font-size:11px}
.fnbox .fn{margin-bottom:2px}.fnbox .fn .nm{font-weight:500;color:var(--in)}
.fnbox .fn .ok{color:var(--ok)}.fnbox .fn .fail{color:var(--dn)}
...
.rec audio{width:100%;height:32px;margin-top:4px}
...
<div class="app">
<header>
 <div class="brand">Lead Journey v2</div>
...
const HL={'CLIENT_INITIATED':'Client hung up','PARTICIPANT_REMOVED':'Agent ended','USER_UNAVAILABLE':'Unreachable','USER_UNRESPONSIVE':'No response','USER_REJECTED':'Rejected','VOICEMAIL_DETECTED':'Voicemail','OUTSIDE_TRIGGER_WINDOW':'Outside window'};
const HB={'CLIENT_INITIATED':'b-ok','PARTICIPANT_REMOVED':'b-ok','USER_UNAVAILABLE':'b-ne','USER_UNRESPONSIVE':'b-wn','USER_REJECTED':'b-dn','VOICEMAIL_DETECTED':'b-wn','OUTSIDE_TRIGGER_WINDOW':'b-ne'};
...
function apply(){
 ...
 st.fil=D.filter(l=>{
  ...
  if(!q)return true;
  const h=[l.id,l.city,...l.calls.map(c=>[c.milestone,c.dispose,c.scenario,c.hangup,c.sentiment,c.dispose_reason].join(' '))].join(' ').toLowerCase();
  return h.includes(q);
...
function prefGrid(obj,label){
 ...
 const nice={'fuel_type':'Fuel','transmission':'Trans','body_type':'Body','color':'Color','rto':'RTO','make':'Make',
  ...
  'min_mileage':'Min KM','max_mileage':'Max KM','no_of_owners':'Owners','features':'Features','city':'City'};
...
function carsHtml(cars,label){
 if(!cars||!cars.length)return'';
 return`<div class="slbl">${esc(label)}</div><div class="acards">${cars.map(c=>
  `<div class="acard"><b>${esc(c.make)} ${esc(c.model)}</b><span class="sm">${c.price?fp(c.price):''}${c.fuel?' · '+esc(c.fuel):''}${c.hub?' · '+esc(c.hub):''}</span></div>`
 ).join('')}</div>`;
}
...
function renderDet(id){
 ...
 const callsH=l.calls.map((c,i)=>{
  ...
  metaH+=carsHtml(mt.assigned_cars,'Assigned cars to pitch');
  if(mt.car_context){
   const cc=mt.car_context;
   metaH+=`<div class="slbl">Car being discussed</div><div class="cf">
    <div><div class="fl">Car</div><div class="fv">${esc(cc.make_model)}</div></div>
    ${cc.price?`<div><div class="fl">Price</div><div class="fv mono">${fp(cc.price)}</div></div>`:''}
    ${cc.km?`<div><div class="fl">KM</div><div class="fv mono">${(+cc.km).toLocaleString()}</div></div>`:''}
    ${cc.year?`<div><div class="fl">Year</div><div class="fv mono">${cc.year}</div></div>`:''}
    ${cc.owner?`<div><div class="fl">Owner</div><div class="fv mono">${cc.owner}</div></div>`:''}
   </div>`;
  }
  if(mt.last_call){
  ...
  if(mt.exchange){
   const ex=mt.exchange;
   metaH+=`<div class="slbl">Exchange car</div><div class="cf">
    <div><div class="fl">Car</div><div class="fv">${esc(ex.make)} ${esc(ex.model)} ${esc(ex.variant||'')}</div></div>
    ${ex.year?`<div><div class="fl">Year</div><div class="fv mono">${ex.year}</div></div>`:''}
   </div>`;
  }
  if(mt.comments){
  ...
  // POST panel
  const pf=[
   {l:'Milestone',v:ML[c.milestone]||c.milestone},{l:'Disposition',v:(c.dispose||'').replace(/_/g,' ')},
   {l:'Disp reason',v:c.dispose_reason},{l:'Sentiment',v:c.sentiment},
   {l:'Budget',v:c.budget?fp(+c.budget):''},{l:'Visit status',v:c.visit_status},
   {l:'Hangup',v:hl},{l:'Scenario',v:scn},{l:'Capability',v:c.capability},
   {l:'Try #',v:c.try_count!=null?String(c.try_count):''},
   {l:'Pitched cars',v:c.pitched_cars,m:1},{l:'Rejected cars',v:c.rejected_cars,m:1},
   {l:'Liked cars',v:c.liked_cars,m:1},{l:'Interested cars',v:c.interested_cars,m:1},
   {l:'TD cars',v:c.td_cars,m:1},{l:'Selected TD slot',v:c.selected_td_slot?ft(c.selected_td_slot):''},
   {l:'Preferred hub',v:c.preferred_hub},{l:'Updated city',v:c.updated_city},
   {l:'Rescheduled time',v:c.rescheduled_time?ft(c.rescheduled_time):''},
   {l:'Cancel reason',v:c.cancel_reason},{l:'NDOA',v:c.ndoa?ft(c.ndoa):''},
  ].filter(f=>f.v);

  const flags=[
   c.car_pitched&&'Pitched',c.interested_for_td&&'TD Interest',c.interested_buying&&'Buying',
   c.interested_sell&&'Wants Sell',c.callback_req&&'Callback',c.wants_td_2d&&'TD>2d',
   c.notify_supply&&'Supply Alert',c.loan_interest&&'Loan',c.conversion&&'Converted',
  ...
  if(c.functions&&c.functions.length){
   fnH=`<div class="fnbox">${c.functions.map(f=>`<div class="fn"><span class="nm">${esc(f.name)}</span> <span class="${f.success?'ok':'fail'}">${f.success?'✓':'✗'}</span> <span style="color:var(--tf);font-size:10px">${esc(f.ts)}</span></div>`).join('')}</div>`;
  }else{fnH='<div style="color:var(--tf);font-style:italic;padding:8px">No functions called</div>'}

  // Transcript panel
  let txH=c.transcript?`<div class="txbox">${esc(c.transcript)}</div>`:'<div style="color:var(--tf);font-style:italic;padding:8px">No transcript</div>';
  if(c.recording)txH+=`<div class="rec"><audio controls preload="none" src="${esc(c.recording)}"></audio><a href="${esc(c.recording)}" target="_blank">Open recording ↗</a></div>`;
  ...
  return`<div class="call ${cls}">
   <div class="ch" data-cid="${cid}">
    ...
    <span class="cscn">${esc(scn)}</span>
    <span class="cdisp">${esc(tr((c.dispose||'').replace(/_/g,' '),100))}</span>
    <span class="caret">▶</span>
   </div>
   <div class="cb">
    <div class="ctabs">
     <div class="ctab on" data-p="meta_${cid}">Before Call</div>
     <div class="ctab" data-p="post_${cid}">After Call</div>
     <div class="ctab" data-p="fn_${cid}">Functions</div>
     <div class="ctab" data-p="tx_${cid}">Transcript</div>
    </div>
    <div class="cpanel on" id="meta_${cid}">${metaH}</div>
    <div class="cpanel" id="post_${cid}">${postH||'<div style="color:var(--tf);font-style:italic;padding:8px">No post-call data</div>'}</div>
    <div class="cpanel" id="fn_${cid}">${fnH}</div>
    <div class="cpanel" id="tx_${cid}">${txH}</div>
   </div>
  </div>`;
 }).join('');
...
def main():
    ...
    if not csv_path or not csv_path.exists():
        print("Usage: python generate_viewer_v2.py <input.csv> [output.html]")
        sys.exit(1)
